...
instruccionLoop
    : LOOP_
        { $<cent>$ = si; /*Inicio de cada iteracion*/ }
      instruccion UNTIL_ expresion
        { if ($5.tipo != T_ERROR && $5.tipo != T_LOGICO)
            yyerror(E_LOOP_LOGICAL);
          $<cent>$ = creaLans(si); /*Salto al final del bucle*/
          emite(EIGUAL, crArgPos($5.pos), crArgEnt(TRUE), crArgNul());
        }
      STEP_ expresion
        { if ($8.tipo == T_ARRAY)
            yyerror(E_LOOP_STEP);
